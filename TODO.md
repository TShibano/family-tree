...
家系図にふさわしい日本の伝統的な雰囲気に仕上げる。

### 変更内容

#### 1. フォント
- **名前**: ヒラギノ明朝（`/System/Library/Fonts/ヒラギノ明朝 ProN.ttc`）— 格式ある印象
- **生年月日**: 同フォントの小さめサイズ
- フォールバック: ヒラギノ角ゴシック → システムデフォルト

#### 2. 配色（和色）
- **背景**: 生成色（きなりいろ）`#F5F0E8` — 和紙のような温かみ
- **男性ブロック**:
  - 塗り: 白藍（しらあい）`#C1D8EC` — 薄い藍色
  - 枠: 藍色（あいいろ）`#2E4F6F`
- **女性ブロック**:
  - 塗り: 桜色（さくらいろ）`#FDEFF2` — 淡い桜
  - 枠: 蘇芳（すおう）`#8E354A`
- **婚姻線**: 朱色（しゅいろ）`#C53D43` — 赤の紐のイメージ
- **親子線**: 墨色（すみいろ）`#595857`
- **テキスト**: 墨 `#2B2B2B`

#### 3. ブロック形状
- 角丸矩形に変更（`rounded_rectangle`）— 柔らかい印象
- 枠線をやや細め（1.5〜2px）
...
- `frame_drawer.py` のみ（描画パラメータ・色・フォント・描画処理の変更）

### 実装ステップ

1. 色定数を和色に変更
2. フォント候補リストにヒラギノ明朝を追加
3. `draw.rectangle` → `draw.rounded_rectangle` に変更
4. フォントサイズ微調整
5. テスト実行・動画再生成で確認
